...
> **Note on the spike:** only this ADR is merged to `main`. The spike code and its full metrics report it refers to live in [PR #1875](https://github.com/WXYC/Backend-Service/pull/1875) (branch `critic-review-search-spike`, directory `spike/critic-review-search/`) and are intentionally not merged. Every in-repo `spike/…` path referenced below resolves against that branch, not `main`.
