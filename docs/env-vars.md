...
- `BACKFILL_LEG` — Selects the source leg for `library-identity-backfill`. Locked values: `S1` (default — Backend `canonical_entity_id`, sub-PR 2.0) or `S2` (LML-derived artist identity from Backend's mirrored `artists` columns + provenance from LML `reconciliation_log`, sub-PR 2.1). Anything else is rejected at job start. Future legs (2.2a, 2.2b, 2.3) extend this enum.
- `DATABASE_URL_DISCOGS` — Required for `BACKFILL_LEG=S2` (the S2 leg loads LML's `entity.reconciliation_log` once at job start to derive per-row `(method, confidence)`). Same connection string as the artist-identity ETL uses.
