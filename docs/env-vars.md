...
- `ROM_INTERNAL_KEY` _required for `/internal/banned-fingerprints` + `/auth/check-request-ban`_ — Shared secret matched against the `X-Internal-Key` header on the request-line ban surface (BS#1261). Distinct from `ETL_NOTIFY_KEY` so rotating one doesn't disrupt the other — different caller (request-o-matic), different blast radius. The `/internal/banned-fingerprints` CRUD endpoints reject the request when the header value doesn't match. On `/auth/check-request-ban`, the header is intended for rate-limit bucketing only (not authorization); the JWT/fingerprint payload determines the response shape. Generate with `openssl rand -base64 32`.
