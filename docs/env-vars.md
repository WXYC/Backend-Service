...
| 5     | Batch/backfill enrichment         | 29000                                            | 28000 (timeoutMs − 1000)                                                                 | `metadata-service`, `enrichment-worker`, and every `jobs/*` bulk/backfill caller                                                                                                                                                              |

Env levers:

- `LML_CLASS1_TIMEOUT_MS` (default `3000`), `LML_CLASS2_TIMEOUT_MS` (default `5000`), `LML_CLASS2_BUDGET_MS` (default `4000`), `LML_CLASS3_TIMEOUT_MS` (default `8000`), `LML_CLASS4_TIMEOUT_MS` (default `5000`), `LML_CLASS5_TIMEOUT_MS` (default `29000`, `budgetMs` derives as `timeoutMs − 1000`) — per-class defaults, read fresh by `resolveLmlPolicy` on every call (not cached at module scope beyond the module-load-time `LML_CALLER_POLICY` build). BS#1842: the final `budgetMs` for every caller is sanitized after class defaults AND per-caller overrides are applied — a `budgetMs <= 0` (e.g. `LML_CLASS5_TIMEOUT_MS` below `1000`) or a `budgetMs >= timeoutMs` (e.g. an `LML_CLASS2_BUDGET_MS` set at or above `LML_CLASS2_TIMEOUT_MS`) warns and is treated as "no budget header" instead of sending a nonsensical `X-Caller-Budget-Ms` value.
...
### LML-client limiter queue deadline + circuit breaker (BS#1748)

The shared client's process-wide `defaultLimiter` (`shared/lml-client/src/index.ts`, BS#906/G4) used to do an unbounded, un-timed `Semaphore(5).acquire()` before the HTTP span even opened — under sustained LML slowness, five permits draining at LML's pace could queue callers for minutes with no deadline and no visible HTTP span (the "invisible 225 s" the BS#1748 diagnosis measured). Two mechanisms now bound that:

- **Bounded queue wait** (`Semaphore.acquire(maxWaitMs)`): a call not admitted within the deadline is shed — rejects with `LmlSheddedError('queue_deadline_exceeded', ...)` — instead of continuing to queue.
- **Circuit breaker** (`LmlCircuitBreaker`): a simple consecutive-failure/cooldown breaker (not a rolling error-rate window). Opens after `failureThreshold` consecutive shed/failed calls, fast-failing every subsequent call with zero queue wait; after `resetTimeoutMs`, the next call is admitted as a single half-open probe — success closes the breaker, failure reopens it and restarts the cooldown.

Both are opt-in fields on `createLmlLimiter`'s config (`queueDeadlineMs` / `breaker`); omitting either preserves the pre-BS#1748 unbounded-wait shape. Only the runtime `defaultLimiter` enables them — the backfill/job-level limiters below (`BACKFILL_LML_*`) intentionally keep the original shape, since those jobs have no human-facing latency budget and are meant to queue rather than shed.

A shed surfaces as an `LmlSheddedError` (extends `LmlClientError`, `statusCode: 503`) — every existing catch arm that already treats an LML failure as "leave the row in its pre-terminal state for the recovery sweep" needs no changes: `apps/enrichment-worker/enrich.ts` leaves the row `enriching` for the C6 stranded-claim sweep (#895, now a live safety net) to revert to `pending`; `apps/backend/services/metadata/enrichment.service.ts`'s catch arm writes synthesized search URLs without stamping `metadata_attempt_at`; `library.service.ts`'s `enrichWithArtwork` already logs-and-continues on any `Promise.allSettled` rejection. A shed never produces a terminal `failed` status.

- `LML_LIMITER_QUEUE_DEADLINE_MS` (default `15000`) — Max ms the default limiter's Semaphore will queue an admission before shedding. Sized so `queueDeadlineMs + TIMEOUT_MS` (the module's 30 s fire-and-forget default) stays comfortably under the enrichment-worker sweep's `STRANDED_TTL_SECONDS` floor (60 s) — a shed must never take as long as an actual strand would. A module-load `console.warn` fires if a misconfigured override would erase that headroom (mirrors the `sanitizeBudgetMs` warn-on-invalid convention, BS#1842).
- `LML_CIRCUIT_BREAKER_THRESHOLD` (default `5`) — Consecutive shed/failed calls before the breaker opens.
- `LML_CIRCUIT_BREAKER_RESET_MS` (default `30000`) — Cooldown before an open breaker admits a half-open probe.

**CI-safety overrides:** several existing integration specs (e.g. `tests/integration/metadata.spec.js`) deliberately fire multiple consecutive simulated LML 500s against this SAME process-wide `defaultLimiter` to exercise fallback behavior. A default-sized breaker threshold would risk tripping mid-suite and shedding calls for later, unrelated tests. Both `dev_env/docker-compose.yml`'s `backend` service and `.github/workflows/test.yml`'s `Start services` step therefore set `LML_CIRCUIT_BREAKER_THRESHOLD` far above anything the integration suite could plausibly trip — the same BS#955 precedent that already overrides `LML_CLIENT_MAX_CONCURRENT`/`LML_CLIENT_RATE_PER_MIN` for the same reason. Pinned by `tests/unit/scripts/lml-limiter-test-env.test.ts` so a future limiter env var (or a lowered CI override) can't silently regress the suite.
