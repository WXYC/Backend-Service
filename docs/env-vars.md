...
- `BACKFILL_FLOOR_RECENCY_DAYS` (default `7`) — Recency exemption from the floor (BS#1591 design decision 5): rows younger than this many days are always eligible, so when BS#895 re-scopes this cron into the hourly gap-recovery sweep, consumer-missed rows of below-floor artists stay sweepable instead of being permanently stranded. `0` disables the exemption — only sensible while the cron remains a pure historical drain. Non-negative integer; anything else throws at startup.
