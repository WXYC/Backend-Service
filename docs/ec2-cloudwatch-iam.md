...
> ```
>
> An `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY` **pair** tops the AWS SDK's default credential chain for the whole process and shadows the instance role, so a correctly-attached policy is simply never consulted. That is BS#2518: the SES-only `no-reply-sender` credential sat under those two reserved names and kept `WXYC/BackendService` from existing as a namespace for 105 days, while this doc's remedy was already in place. `sts get-caller-identity` should report `assumed-role/wxyc-ec2-backend`, not a `user/`.
>
> The detector below flags two further names, `AWS_SESSION_TOKEN` and `AWS_PROFILE`, whose mechanics are weaker — do not read a warning about either as proof you have found the cause. `AWS_SESSION_TOKEN` alone does not satisfy the env-credential provider, which needs the id/secret pair, so the chain still falls through to IMDS; `AWS_PROFILE` sits _below_ the env provider and only selects which profile `fromIni`/`fromSSO` reads, resolving to nothing on a host with no `~/.aws/config`. Neither is ever correct on a WXYC host — both mean someone exported a credential set into a process meant to run as its instance role — which is why they are reported, but if `PutMetricData` is returning `AccessDenied` and the only reserved name present is one of these two, keep looking.
>
