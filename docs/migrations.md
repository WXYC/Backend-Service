...
Six `timestamp with time zone` markers record "this row was attempted by job X" so subsequent passes can target only the rows that still need work, without confusing tried-and-no-match for tried-and-failed. The shared shape: stamped on responded outcomes (success-with-match OR success-no-match), left NULL on transient failures (the `.catch` arm deliberately omits the write), read by a cron or warm walker that re-attempts the NULL set. (Two markers deviate from parts of that shape and say so in their bullets: `library.unresolved_attempted_at` is stamped on **no-match only** — success is recorded by a `library_identity` row — and is re-attempted **manually**, with no cron/walker backstop.)
