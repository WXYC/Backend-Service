...
- `concert_performers.artist_resolve_attempted_at` — binds ONLY the Phase-D LML arm (the offline verify-before-mint resolver for support-act performers absent from the WXYC library — the `concerts.artist_resolve_attempted_at` analog, one level down). Migration 0128 (BS#1759, parent #1618). Byte-identical semantics: stamped when LML RESPONDED for the row's `raw_name` (resolved / ambiguous / not_found), left NULL on `escalation_unavailable` verdicts and transport errors so those rows stay immediately retryable. The Phase-B pure-SQL strict/alias resolve arm carries **no** marker, matching today's `concerts-artist-resolver`'s treatment of headliners. This migration is substrate-only — no writer exists yet; the consuming resolver job lands in a later slice of the On Tour supports series.
