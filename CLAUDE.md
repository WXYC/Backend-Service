...
| `@wxyc/triangle-shows-etl`                  | `jobs/triangle-shows-etl/`                  | Nightly pull ETL (BS#1589): mirror the [triangle-shows](https://github.com/WXYC/triangle-shows) concert calendar API into `venues`/`concerts` for the 16 Triangle venues the venue-events-scraper doesn't cover (5 double-covered slugs are excluded with startup drift assertions). Keyed on `(source='triangle_shows', source_id='<venue_slug>:'+source_key)` — never bare `source_key`, which is only unique per venue. Full snapshot each run (`dedup=false&include_removed=true` + back-dated `start`); `status` and `removed_at` are source-authoritative and refresh in both directions, unlike `rhp_scrape` (see the job README). Requires `TRIANGLE_SHOWS_URL`. Default schedule `5 5 * * *` UTC — between the venue-events-scraper (05:00) and the concerts-artist-resolver (05:15) so new rows get artist-resolved the same night.           |
