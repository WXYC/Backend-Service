...
5. **migrate-dryrun** -- Only when `db-init` paths change (migrations, schema, `init-db.mjs`, etc.). Restores the most recent automated RDS snapshot to a sandbox DB instance, runs `node scripts/dryrun-migrate.mjs` against it, asserts exit 0, tears the sandbox down. Catches preconditions that depend on prod data shape (e.g. the `RAISE EXCEPTION` guards added per WXYC/Backend-Service#705) at PR-review time rather than at deploy time. The script reuses `dev_env/format-pg-error.mjs` (#725) so the underlying Postgres error fields surface in the CI log on failure. To re-test against a fresher snapshot, trigger an on-demand snapshot via the AWS RDS console and rerun the workflow. Provisioning prerequisites (ops, one-time, all idempotent via `scripts/provision-dryrun-aws.mjs`): IAM policy attached to the existing `AWS_ACCESS_KEY_ID` user with `rds:DescribeDBSnapshots`, `rds:DescribeDBInstances`, `rds:RestoreDBInstanceFromDBSnapshot`, `rds:DeleteDBInstance`, `rds:AddTagsToResource`, plus `ec2:DescribeManagedPrefixLists`, `ec2:GetManagedPrefixListEntries`, `ec2:ModifyManagedPrefixList` (the EC2 actions scoped by tag `wxyc:purpose=gha-egress` to the prefix list shards described next); 5 managed prefix list shards (`wxyc-gha-egress-{1..5}`, each capacity 1000) populated with the aggregated GitHub Actions IPv4 egress CIDRs from `api.github.com/meta` and re-synced weekly by `.github/workflows/sync-gha-prefix-list.yml`; security group `SG_DRYRUN_GHA` allowing port 5432 from each shard via separate ingress rules; repo secrets `PROD_DB_ID`, `PROD_DB_NAME`, `PROD_DB_USERNAME`, `PROD_DB_PASSWORD`, `SG_DRYRUN_GHA`. The shard count was forced by AWS's hard cap of 1000 entries per PL — aggregated GHA IPv4 is ~3300 entries; sharding by SHA-256(cidr) mod N keeps per-shard diffs stable on weekly syncs. See WXYC/Backend-Service#726 (initial gate setup) and #756 (shift from inline CIDR allowlist to managed prefix list shards).
