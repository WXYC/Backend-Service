...
| `@wxyc/flowsheet-linked-reenrichment`       | `jobs/flowsheet-linked-reenrichment/`       | One-shot re-enrichment (BS#1638) of the BS#1443 `enriched_no_match` **linked** cohort (22,773 rows, frozen four-clause predicate: `metadata_status='enriched_no_match' AND album_id IS NOT NULL AND artist_name IS NOT NULL AND add_time < '2026-06-16T17:53:53Z'`). **Lane A (shipped)**: pure SQL, zero LML — flip cohort rows whose `album_id` already has a _populated_ `album_metadata` row (`discogs_url OR artwork_url NOT NULL`) to `enriched_match`, batched + `ANALYZE` after (~15,231 rows / 523 albums). **Lane B (chained follow-up PR)**: the ~314 residual albums (308 both-null + 6 absent) through `bulkLookupMetadata` (LML#368, post-LML#784) with a fill-null `album_metadata` UPSERT then a paired flip. Structural donor: `album-level-backfill` (BS#1041). Never sets `pending`, never writes `metadata_attempt_at` (BS#1011/BS#895 untouched — the reason BS#1443's Option 2 was rejected). Dry-run by default (`--execute` to write); out-of-band partial-index pre-flight in the README.                                                                                                                                                                                                                                                                                 |
