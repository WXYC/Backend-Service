...
1. **Load candidates** (`targets.ts`): upcoming (`starts_on >= (now() AT TIME ZONE 'America/New_York')::date` — the venue-local Eastern date the read path also windows on, not server-clock `CURRENT_DATE`), non-tombstoned rows with BOTH id columns NULL, where the attempt-at marker is NULL or older than the no-match TTL.
