...
## `--recheck` — on-demand re-drain of the resolved-compilation cohort

Set `RECHECK=true` to run a single `va`-cohort drain over every library row this job has previously visited (`unresolved_attempted_at IS NOT NULL`), **ignoring** `UNRESOLVED_RETRY_DAYS` entirely. Use this after a deliberate LML matcher improvement, to re-ask rows without waiting out the TTL — the job doesn't poll for matcher improvements on its own ("store improvements propagate by deliberate re-drain, not polling"). Reuses the same paging/limiter/dry-run machinery as a normal run; `RECHECK=true` takes priority over the two-cohort drain and skips `non_va` entirely (compilations are inherently `va`).
