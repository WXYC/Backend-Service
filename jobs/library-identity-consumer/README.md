...
3. `kind: 'compilation'` whose per-track matcher ran this batch (`tracks_attempted === true`, response carries `tracks_contract_version === 1`) — a **resolved exit**, counted as `rows_resolved_compilation`. `writeCompilationTracks` (page-level, not per-result) fetches the page's `compilation_track_artist` rows in one query, resolves each track entry's `resolved_artist_name` to a local `artists.id` via a batched `fold_artist_name` join, and UPSERTs `track_artist_id` / `track_artist_link_confidence` / `track_artist_link_method = 'lml_backfill'` — never overwriting a row already linked by a librarian (`track_artist_link_method = 'librarian'`), and never touching an unchanged row — unchanged rows are filtered app-side before the UPDATE is even issued (the `library_watermark` trigger is `FOR EACH STATEMENT` and fires on `UPDATE 0` for whatever column set it watches, so the SQL-level `IS DISTINCT FROM` guard alone would not keep a no-op re-drain from issuing a statement; the app-side prefilter remains a backstop against wasted statements even though, per the next sentence, none of the four columns it guards can advance the watermark regardless as of BS#2054). A fully-unchanged page issues no UPDATE statement on `compilation_track_artist`, so the CTA write path fires neither the CDC nor the watermark trigger on a no-op re-drain — and as of [BS#2054](https://github.com/WXYC/Backend-Service/issues/2054) (migration 0143) even a page with genuine changes no longer advances the watermark: the CTA `library_watermark` trigger is narrowed to `UPDATE OF <exported columns>` (`id`, `library_id`, `artist_name`, `track_title`), and none of the four columns this UPSERT writes are in that set. A first-time `track_artist_id` resolution — a genuine value change the app-side prefilter above does NOT (and structurally cannot) skip — no longer busts the CTA export's conditional GET either. (The per-batch `unresolved_attempted_at` stamp on `library` is still a genuine row change — `NOW()` — but as of [BS#2052](https://github.com/WXYC/Backend-Service/issues/2052) (migration 0142) the `library` watermark trigger is narrowed to `UPDATE OF <exported columns>`, and `unresolved_attempted_at` is not one of them: it is an internal drain marker no catalog export reads. The stamp no longer advances `library_watermark`, so a `--recheck` sweep that resolves nothing new now leaves the watermark untouched — down from ~13 statements per page pre-BS#1991, to one per-batch stamp post-BS#1991, to zero post-BS#2052.) A CTA row with a NULL `track_position` gets the entry's position written verbatim when exactly one distinct position was offered at that `(artist_name, track_title)` key; two+ conflicting positions skip the position write (logged) without affecting the identity write.
