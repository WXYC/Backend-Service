...
- A per-artist fetch failure (transport error, LML/Discogs error) is skipped and left retryable — no attempt-at marker, the `image_url IS NULL` predicate is the retry substrate. An _expected_ failure (`LmlClientError`: a 404 for a stale/absent Discogs id, or a 5xx/timeout during an outage) is counted + warn-logged but **not** captured to Sentry — with no negative cache, an unconditional capture would fire one event per night per bad id (and one per headliner during a full LML outage). Only a genuinely-unexpected error escalates.
