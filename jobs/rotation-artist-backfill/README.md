...
| `SENTRY_TRACES_SAMPLE_RATE`   | `0`        | Default 0 means spans are **not** sent. Gates all spans this job emits: the `op:job.run` parent span (and its catch-block error status — so span-status-based error-rate alerts only fire at non-zero sample rate), the `op:http.client` per-call spans, and the `${JOB_NAME}.run.totals` summary. `captureException` issue events fire regardless of sample rate. |
