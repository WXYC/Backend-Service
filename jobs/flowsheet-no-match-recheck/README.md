...
`job.ts` runs `query.ts`'s `loadCandidates` twice: once at OFFSET 0 for `HEAD_SLICE` rows, once at the BS#2218 cursor for `BATCH_SIZE - HEAD_SLICE` rows (any tail row already covered by the head read is dropped — `orchestrate.ts`'s `excludeCandidateIds`). The two runs' `Totals` are reported together (`mergeTotals`), but the cursor advances using only the tail run's totals — the head slice never occupied a cursor position, so folding it in would advance the cursor past tail rows it never read (see `watermark.ts`'s module doc).

`MEASURED_INFLOW_ROWS_PER_DAY` (40) is 199 new `enriched_no_match` rows over 2026-09-13 → 2026-09-18 (5 days), replayed against `flowsheet.updated_at`. `RUNS_PER_DAY` (4) is the cron cadence — see "Schedule" below. `HEAD_SLICE_COVERAGE_MARGIN` (2) is the safety factor over that average so a heavier play day or a backfill drain still gets same-day head coverage instead of re-creating the queueing this whole job exists to remove. Each is its own exported constant in `query.ts` — not folded into a single magic number — so a `BATCH_SIZE`/cadence resize (BS#2186) recomputes `HEAD_SLICE_DEFAULT` instead of silently invalidating it.
...
| `FLOWSHEET_NO_MATCH_RECHECK_HEAD_SLICE`              | `20`       | BS#2222: rows always read at OFFSET 0, on top of `BATCH_SIZE - HEAD_SLICE` at the cursor — see "HEAD_SLICE derivation" above                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
