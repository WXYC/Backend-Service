/**
 * Concurrency + rate-limit gate for jobs/flowsheet-no-match-recheck (BS#2176).
 *
 * Mirrors jobs/rotation-release-id-backfill/lml-limiter.ts (itself a mirror
 * of jobs/flowsheet-metadata-backfill/lml-limiter.ts) verbatim — same
 * BACKFILL_LML_* env defaults (concurrency=1, rate=20/min) and the same
 * `defaultLmlLimiter` singleton pattern. Each job runs in its own Docker
 * container with its own `.env`, so reusing the same env var NAMES across
 * jobs is safe — they resolve independently per-process. The 2026-05-21
 * incident (BS#994) is the safety story this limiter exists to defend;
 * copying the file keeps this job's blast-radius story identical to its
 * siblings' without coupling any of their build graphs together.
 *
