...
- **An early exit is never reported as a completed run.** The batch loop can end three ways short of the last batch: a `SIGTERM`/`SIGINT` graceful stop, the shared cooperative-pause ceiling, or an error out of the pause itself. All three set `stopped_early: true` in the summary, and the latter two additionally **rethrow, so the process exits non-zero** — without that, a run aborted at batch 3 of ~890 would print the same shape of clean summary and exit 0 as a completed drain, which is indistinguishable to a cron or to an operator reading logs. A budget exhaustion is tagged `live_activity_pause_ceiling_exceeded`; anything else out of the pause is tagged `live_activity_pause_failed`, so nobody is sent to the `LIVE_ACTIVITY_MAX_PAUSE_MS` knob for an unrelated failure.
- **`SIGTERM`/`SIGINT` stop gracefully.** The in-flight batch finishes (its writes are already committed per album) and the run ends before the next one. Resuming is just re-running: the cohort predicate is the cursor.
