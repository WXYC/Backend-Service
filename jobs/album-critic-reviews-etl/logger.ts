/**
 * Observability for album-critic-reviews-etl: Sentry init + JSON logs.
 *
 * Copied from `jobs/album-reviews-etl/logger.ts` (itself copied from
 * `jobs/triangle-shows-etl/logger.ts`, the issue-#538 contract; per-job
 * duplication keeps this job's build graph independent of the long-running
 * services), INCLUDING `captureWarning` — the message-level Sentry signal
 * this job's `nothing_new`/run-guard-adjacent warnings can use. When
 * propagating logger fixes across the job fleet, do NOT "restore verbatim"
 * by deleting it — and note `jobs/flowsheet-metadata-backfill/logger.ts`
 * has separately diverged (tracesSampler work, BS#1457/#1566), so it is not
 * a sync source for cron jobs that default tracing off (this one does).
 */
