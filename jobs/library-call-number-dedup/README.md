...
4. Add the uniqueness constraint, keyed as above. In the **same** PR, map its 23505 onto a 409 on every write path that can now collide — `POST /library` and `PATCH /library/:id` — so the constraint and a clean 409 land together with no window where a duplicate answers 500. That write set grew in BS#2410: `POST /library` accepts an operator-supplied `code_number` with no application-level collision check (single-librarian decision, rotation-import plan D2 — `albumCodeNumberTaken` is deliberately not called there, because a hard 409 would block the deliberate re-use of a lost record's slot), so this constraint is the only thing that will ever enforce uniqueness on the POST path. The mapping cannot be written ahead of the constraint: it is an expression index whose name is not chosen yet, so a dormant handler could neither match it nor be tested.
