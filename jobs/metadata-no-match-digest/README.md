...
- `SES_FROM_EMAIL`, `SES_ACCESS_KEY_ID`, `SES_SECRET_ACCESS_KEY`, `AWS_REGION` -- SES send config (shared with the auth service's transactional email). **Do not set the credential as `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`**: those are the AWS SDK's reserved global names and shadow the EC2 instance role for every AWS call in the process (BS#2518). The transitional fallback that read them is gone, so that spelling now fails the send outright; it still warns while present, because the shadowing does not need this job to read it.
