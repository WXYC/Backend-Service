 ...
 * Tier 1 keeps its singleton; tier 2 only ever sees what tier 1 declined.
 *
...
export type LibraryCandidateRow = LibraryCandidateRowBase;
 ...
 *
 * Two MATERIALIZED CTEs force each `normalize_artist_name` and each
 * `fold_artist_name` to be evaluated exactly once per library row per leg.
 * Inlined, the OR-across-legs predicate re-evaluates the functions in both
 * the WHERE and the projection (up to 12× per row across six keys).
 *
  ...
  const result: unknown = await db.execute(sql`
    ...
    folded AS MATERIALIZED (
      SELECT
        ...
        norm_album_artist,
        norm_alternate,
        ${FOLD_FN}(norm_primary) AS fold_primary,
        ${FOLD_FN}(norm_album_artist) AS fold_album_artist,
        ${FOLD_FN}(norm_alternate) AS fold_alternate
      FROM normalized
    )
...
export const linkSubmissions = async (deps: Partial<LinkDeps> = {}): Promise<LinkTotals> => {
  ...
  const totals: LinkTotals = { linked: 0, linked_exact: 0, linked_relaxed: 0, link_ambiguous: 0, link_unmatched: 0 };
  ...
  const foldedNorms = [...new Set(norms.map((n) => foldArtistName(n)))];
  ...
  for (const [norm, group] of byNorm) {
    ...
    for (const key of new Set([norm, foldArtistName(norm)])) {
