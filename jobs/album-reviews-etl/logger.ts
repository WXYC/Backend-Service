/**
 * Observability for album-reviews-etl: Sentry init + JSON logs.
 *
 * Copied from `jobs/triangle-shows-etl/logger.ts` (the issue-#538
 * contract; per-job duplication keeps this job's build graph independent
 * of the long-running services), INCLUDING its `captureWarning` addition —
 * the message-level Sentry signal this job's fallback-source-key and
 * normalization-anomaly warnings depend on. When propagating logger fixes
 * across the job fleet, do NOT "restore verbatim" by deleting it — and
 * note `jobs/flowsheet-metadata-backfill/logger.ts` has separately
 * diverged (tracesSampler work, BS#1457/#1566), so it is not a sync
 * source for cron jobs that default tracing off.
 */
