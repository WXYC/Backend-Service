...
7. Publish the same rendered content to research-data (`publish.ts`). A publish failure is caught, logged, and counted — never aborts the run; the local file write already succeeded and is this run's durable artifact regardless.
