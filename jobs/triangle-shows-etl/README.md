...
## Clean-headliner extraction (BS#1604) + billing-tail support capture (BS#1758)
...
`headliner.ts`'s `parseBilling` (BS#1758) generalizes `extractHeadliner` into `{ headliner, support[] }` — `extractHeadliner` is now a thin `parseBilling(...).headliner` wrapper, byte-identical to before. The support half captures the SAME billing-tail text the headliner strip used to discard (` w/ X`, ` // X // Y`, ` feat./ft./featuring X`), splitting it further on those same tail delimiters plus a comma **inside the already-isolated tail only** (`X // Y // Z` → `[Y, Z]`; `X w/ Y, Z` → `[Y, Z]`) — never on `&`/`and`/plain `with`/`+`, exactly like the headliner strip. `map.ts`'s `mergeSupportingArtists` unions that billing-derived support with the source's own sparse `support_artists` field (billing first — it's the richer signal), deduping normalize-insensitively via `normalizeFreetextArtist` (keeping the first-seen surface form), dropping any entry that normalizes equal to the chosen headliner, clamping each name to `HEADLINER_MAX` (256) code points, and capping the array at 32 names. `parseBilling` stays DB-free (no `@wxyc/database` import) so the BS#1614 export script keeps working without `DB_*` credentials.

**Visible API change**: `supporting_artists_raw` is projected on the `GET /concerts` DTO; this makes it non-empty for many more `triangle_shows` rows than before (previously populated only from the source's sparse field, now also from the billing tail). Usually richer, but not strictly additive: `mergeSupportingArtists` dedupes normalize-insensitively, drops any entry equal to the headliner, and caps at 32 names, so a given row's array can also be reshaped or shortened relative to the raw union. Either way it's non-breaking — iOS renders it as a plain comma-joined `"with X, Y, Z"` list (`BoxOfficeTicketPresenter.subline`) with no assumption about element count, so any reshaping is a display detail, not a contract change; dj-site does not yet reference the field.
