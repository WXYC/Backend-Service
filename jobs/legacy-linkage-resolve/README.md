...
Folding the count and the UPDATE into one CTE removes both gaps at once: a concurrent write either committed before the statement's snapshot (already a cohort member, gets updated) or after it (invisible to this run, picked up next slot). `resolved === candidates` is true by construction under the unique index, not by luck of timing, so a shortfall is a genuine "the UPDATE isn't draining what its own count sees." A zero-candidate run (or a dry run, which never reaches the UPDATE) never alerts.
