...
**`album_id IS NULL` belongs on the UPDATE, not just the candidate SELECT (BS#2071).** Each pass's single-statement CTE selects its `cohort` with `album_id IS NULL`, then its `upd` UPDATE repeats that same `album_id IS NULL` test in its own WHERE clause — not a redundant restatement. Under READ COMMITTED, if another session updates a `cohort` row and commits while this UPDATE is still waiting on that row's lock, Postgres's EvalPlanQual re-evaluates the UPDATE's own WHERE clause against the row's newly-committed version before writing it. Without the repeated `album_id IS NULL`, that re-check still passes no matter what the concurrent writer just set `album_id` to, so this job silently overwrites it — `apps/backend/services/flowsheet.service.ts`'s `updateEntry` writes `album_id` without touching `legacy_release_id`, so an MD picking an album for a flowsheet entry while this job is mid-run is exactly this shape. With the repeated predicate, the re-check fails once `album_id` is no longer NULL, so the row drops out of `upd` (it's still counted in `cohort`, which is what makes a concurrent write like this show up honestly as a `resolved < candidates` shortfall instead of a silent clobber).
...
Since [BS#2064](https://github.com/WXYC/Backend-Service/issues/2064) the job _does_ write a `cronjob_runs` row, and the distinction matters: that row is a **liveness heartbeat**, read only to report how long the job was away. **Nothing may ever read it back into a repair predicate.** A `last_run` filter on either cohort would permanently strand any row whose `library` row landed during a window the job missed — which is the exact bug this job exists to prevent. Both passes are anti-joined on `album_id IS NULL` and nothing else; `tests/unit/jobs/legacy-linkage-resolve/job.test.ts` pins each statement's full rendered text against an exact-match allowlist (BS#2071 — flipped from a denylist regex, which only catches spellings someone thought to enumerate), so any added predicate fails the test.

**`cohort` selects `DISTINCT` id.** Under today's unique index on `library.legacy_release_id` the join can't fan out, so `DISTINCT` is redundant against the current schema — but it's free against this job's small cohort, and it forecloses a specific failure mode: if that index were ever dropped, a plain (non-distinct) `SELECT` would count one row per matching `library` join partner rather than one per candidate row, inflating `candidates` and tripping the drain check's warning on every run afterward (every 30 minutes, indefinitely) on a job doing nothing wrong. Kept as cheap insurance rather than removed as dead weight.
...
**(c) Drain check.** Each pass measures `candidates` and `resolved` inside a single data-modifying CTE — one statement, one Postgres snapshot — rather than as two separate `db.execute` calls. A shortfall (`resolved < candidates`) is logged and reported to Sentry as a warning; it is evidence the UPDATE didn't drain everything its own count saw, not proof of one. Two earlier shapes got this wrong, in opposite directions, and the current shape narrows but does not eliminate the gap:

- The original BS#2064 shape compared a _pre_-UPDATE COUNT against `resolved` from a separate UPDATE statement, and reasoned that `library.legacy_release_id`'s unique index makes them agree. That reasoning only holds under a single snapshot; two separate statements are two separate snapshots, so anything that removed a row from the cohort in between — a concurrent `broken-fk-recovery` one-shot run, or a hand-run `flowsheet-etl`/`rotation-etl` repair pass (Phase 3 unscheduled their crontab entries but left `resolveAlbumIds` invocable for Phase 6a, gated behind `LEGACY_ETL_ALLOW_BACKWARDS_WRITE=1` — the set of things that can issue this semantically identical UPDATE concurrently is three, not one, though two of the three require that env var deliberately set) issuing it, a DJ deleting the entry, an MD editing it and picking an album, an MD deleting the `library` row a candidate joins to — read as `resolved < candidates` on a run where everything worked.
- BS#2071's first attempt fixed that by re-running the COUNT _after_ the UPDATE and comparing against that instead. That closed the "before" half of the race but opened the "after" half: the pre-UPDATE COUNT and the post-write recount are still two separate statements, two separate snapshots, and a `library` row landing in _that_ gap — most plausibly `library-etl`, which shares this job's exact `*/30 * * * *` slot, not a rare event — showed up in the recount as an unresolved candidate the pass never had a chance to touch. That is a **more common** false alarm than the one it replaced, not a fix.

Folding the count and the UPDATE into one CTE removes both of those gaps: a concurrent write either committed before the statement's snapshot (already a cohort member, gets updated) or after the statement finishes (invisible to this run, picked up next slot) — neither shows up as a shortfall. One snapshot does **not** mean `resolved === candidates` is guaranteed, though: a row already inside `cohort` can still be concurrently UPDATEd or DELETEd _while the statement is running_. Postgres's own concurrency control (EvalPlanQual, under READ COMMITTED) re-checks the UPDATE's WHERE clause against that row's newly-committed version before writing it — a concurrent DELETE removes the row out from under the UPDATE entirely, and a concurrent UPDATE that changes a column the WHERE clause tests (this is exactly why the `upd` CTE repeats `album_id IS NULL`, not just `cohort` — see the "flowsheet UPDATE clobbers a concurrent write" note above) can make the re-checked qual fail. Either way `resolved` comes back one lower than `cohort` counted, for a run where the job did everything it could legitimately still do. A shortfall is real evidence to look at, never something the docs should claim is impossible.

A plain transaction around separate COUNT/UPDATE statements would not have fixed either shape: Postgres defaults to READ COMMITTED, where each statement in a transaction takes its own fresh snapshot. `REPEATABLE READ` pins one snapshot for the whole transaction but then risks the UPDATE failing with a serialization error under concurrent writers — a single statement is the only shape that gets "one snapshot" without that trade.
