 ...
 * gate, and the CI-enforced guard is
 * `tests/unit/routes/album-reviews-permissions.route.test.ts`.
 *
 * Prerequisites (the e2e Docker profile):
 ...
 *   - the fixture accounts' passwords set — `npm run e2e:setup-users` is a
 *     SEPARATE step, not part of `npm run e2e:env`, and `test_dj1`'s password
 *     comes from it.
 *
 * Sequence: npm run e2e:env && npm run e2e:setup-users && npm run test:e2e
 * Run just this file: npm run test:e2e -- tests/e2e/album-reviews-pipeline.test.ts
 */
...
type EtlTotals = {
  ...
  unchanged: number;
  linked: number;
  // Tier split added by the link-pass widening that merged after PR #1679
  // (linked = linked_exact + linked_relaxed).
  linked_exact: number;
...
/**
 * Exchange a better-auth *session* token for the asymmetric-signed JWT the
 * backend verifies against JWKS. The bearer plugin's session token is a
 * different mechanism and would fail `requirePermissions`'s jose verification,
 * so every sign-in path below funnels through here (the `getAdminToken`
 * distinction documented in `tests/utils/anonymous_auth.js`, inverted: that
 * helper wants the session token, this one wants the JWT).
 */
async function exchangeForJwt(sessionToken: string): Promise<string> {
 ...
 * `auth_member.role` puts a real station role on the JWT. No file under
 * `tests/e2e/` did a password sign-in before this one; the shape is ported from
 * the integration helper `getAdminToken` (`tests/utils/anonymous_auth.js`).
 *
 * The password is set by `npm run e2e:setup-users`, which is NOT part of
 * `npm run e2e:env` — if this throws a 401, that step was skipped.
 */
...
/** `test_dj1` — seeded with `auth_member.role = 'dj'` (dev_env/seed_db.sql). */
let djToken: string;
...
describe('read step (GET /album-reviews)', () => {
   ...
   *
   * The anonymous case INVERTS what PR #1679 asserted: it built this as an
   * anonymous-auth surface and pinned 200. Under the gate it is 403, and that
   * inversion is the whole point of the change.
   */
  ...
  it('returns 403 for a real anonymous session (valid JWT, no role claim)', async () => {
    const res = await fetch(`${BASE_URL}/album-reviews`, {
  ...
  it('returns 200 for a dj-role token (test_dj1)', async () => {
    const res = await fetch(`${BASE_URL}/album-reviews`, {
      headers: { Authorization: `Bearer ${djToken}` },
    });
    expect(res.status).toBe(200);
    expect(Array.isArray((await res.json()).album_reviews)).toBe(true);
  });
