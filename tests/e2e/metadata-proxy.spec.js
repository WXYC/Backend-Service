...
beforeAll(async () => {
  ...
  if (!authToken) {
    console.warn(`Could not obtain anonymous auth token from ${BETTER_AUTH_URL}. Proxy tests that require auth will be skipped.`);
  }
...
describe('Metadata Proxy E2E', () => {
  describe('GET /proxy/metadata/album', () => {
    ...
    test('returns Discogs metadata for known artist and album', async () => {
      ...
      const res = await withAuth(
        request(baseUrl)
          ...
          .query({ artistName: 'Stereolab', releaseTitle: 'Dots and Loops' })
      ).expect(200);
    ...
    test('returns search URLs even for unknown artists', async () => {
      ...
      const res = await withAuth(
        request(baseUrl)
          .get('/proxy/metadata/album')
          .query({ artistName: 'xyznonexistent12345' })
      ).expect(200);
    ...
    test('returns 400 without artistName', async () => {
      ...
      await withAuth(
        request(baseUrl).get('/proxy/metadata/album')
      ).expect(400);
    });
  });
  ...
  describe('GET /proxy/metadata/artist', () => {
    ...
    test('returns 400 without artistId', async () => {
      ...
      await withAuth(
        request(baseUrl).get('/proxy/metadata/artist')
      ).expect(400);
    });
