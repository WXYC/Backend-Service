...
describe('real better-call round-trip (BS#2558 re-serialization claim)', () => {
  ...
  // Type-only `import()` references for these paths don't resolve under
  // `tests/tsconfig.json`'s `moduleResolution: "Node"` (it ignores
  ...
  const distDir = path.dirname(require.resolve('better-call'));
  ...
  // eslint-disable-next-line @typescript-eslint/no-require-imports -- `better-call/node`'s public export has no "require"-condition-resolvable types under this file's tsconfig (see the comment above); requiring it here (not `import`) matches app.ts's actual runtime resolution path.
