...
describe('the REAL mock, allowlist and schema', () => {
  ...
  it('would flag the BS#2409 regression if rotation.format_id were not allowlisted', () => {
    // The acceptance case from #2448. `rotation.format_id`/`label_id` sit on
    // the allowlist here because the double still lacks them on this branch;
    // when they are added (PR #2443 does exactly that), the allowlist entries
    // become stale and this guard forces their deletion. Either way the
    // column is covered — this asserts the first half.
    const withoutRotation: DbMockAllowlist = {
      ...DB_MOCK_ALLOWLIST,
      missingColumns: Object.fromEntries(
        Object.entries(DB_MOCK_ALLOWLIST.missingColumns).map(([table, columns]) => [
          table,
          table === 'rotation' ? columns.filter((c) => c !== 'format_id' && c !== 'label_id') : columns,
        ])
      ),
    };
    const findings = compareDbMock(realTables, realDoubles, withoutRotation);
