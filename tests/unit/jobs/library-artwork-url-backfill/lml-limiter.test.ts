// Unit tests for jobs/library-artwork-url-backfill's rate-limit gate
// (BS#1911 review, third commit). Mirrors
// tests/unit/jobs/library-canonical-entity-backfill/lml-limiter.test.ts with
// job-scoped env names (LIBRARY_ARTWORK_URL_BACKFILL_*) and this job's own
// defaults (1 permit, 50/min).

import { Semaphore, TokenBucket, createLmlLimiter } from '../../../../jobs/library-artwork-url-backfill/lml-limiter';

describe('jobs/library-artwork-url-backfill/lml-limiter (BS#1911 review, third commit)', () => {
  describe('Semaphore', () => {
  ...
  describe('createLmlLimiter', () => {
    ...
    it('uses defaults (1 permit, 50/min) when env vars are unset', () => {
      process.env = { ...originalEnv };
      delete process.env.LIBRARY_ARTWORK_URL_BACKFILL_MAX_CONCURRENT;
      delete process.env.LIBRARY_ARTWORK_URL_BACKFILL_RATE_PER_MIN;
    ...
    it('reads LIBRARY_ARTWORK_URL_BACKFILL_MAX_CONCURRENT and LIBRARY_ARTWORK_URL_BACKFILL_RATE_PER_MIN from env', () => {
      process.env = {
        ...originalEnv,
        LIBRARY_ARTWORK_URL_BACKFILL_MAX_CONCURRENT: '3',
        LIBRARY_ARTWORK_URL_BACKFILL_RATE_PER_MIN: '120',
      };
    ...
    it('falls back to default on non-positive or unparseable env values, with a warn', () => {
      process.env = {
        ...originalEnv,
        LIBRARY_ARTWORK_URL_BACKFILL_MAX_CONCURRENT: '0',
        LIBRARY_ARTWORK_URL_BACKFILL_RATE_PER_MIN: 'banana',
      };
      ...
      // prefix is "lml.client:", not a job-local "lml-limiter:" prefix) —
      // match on the env var name/value only, not the prefix, so this
      // doesn't pin an implementation detail owned by the shared client.
      expect(warn).toHaveBeenCalledWith(expect.stringContaining('LIBRARY_ARTWORK_URL_BACKFILL_MAX_CONCURRENT=0'));
      expect(warn).toHaveBeenCalledWith(expect.stringContaining('LIBRARY_ARTWORK_URL_BACKFILL_RATE_PER_MIN=banana'));

      warn.mockRestore();
    });

    it('rejects partial-parse strings like "20banana" (no silent coercion)', () => {
      process.env = { ...originalEnv, LIBRARY_ARTWORK_URL_BACKFILL_MAX_CONCURRENT: '20banana' };
      const warn = jest.spyOn(console, 'warn').mockImplementation(() => {});
      ...
      expect(warn).toHaveBeenCalledWith(
        expect.stringContaining('LIBRARY_ARTWORK_URL_BACKFILL_MAX_CONCURRENT=20banana')
      );
    ...
    it('explicit config overrides env vars', () => {
      process.env = {
        ...originalEnv,
        LIBRARY_ARTWORK_URL_BACKFILL_MAX_CONCURRENT: '999',
        LIBRARY_ARTWORK_URL_BACKFILL_RATE_PER_MIN: '999',
      };
