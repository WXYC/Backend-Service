/**
 * Unit tests for jobs/concerts-artist-lml-resolver targets.ts (BS#1614).
 *
 * The headliner {@link RoleTarget}: candidate predicate shape, the FK
 * loop-close singleton check, and the SET-clause contracts of both write
 * arms. The load-bearing pin is the FK-TIE case: `artists.
 * discogs_artist_id` has NO unique constraint (duplicates exist in the
 * wild via the identity ETL), and a broken singleton check would silently
 * FK the concert to an arbitrary one of the duplicates — the exact
 * mislabel class the strict resolver's `LIMIT 2` collapse exists to
 * prevent. Here the Discogs id must still land while the FK stays NULL.
 *
