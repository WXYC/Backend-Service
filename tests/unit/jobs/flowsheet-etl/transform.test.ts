...
describe('flowsheet-etl transform', () => {
  ...
  // Folded from the former tests/unit/jobs/flowsheet-etl/job.resolveArtistName.test.ts
  // (BS#2119 PR 0): isMessageEntryType/resolveArtistName/resolveMessage moved from
  // job.ts (which self-invokes its ETL `run()` at module scope) into this pure,
  // DB-free module. Regression cover for #1287: the ETL must preserve the verbatim
  // ARTIST_NAME text tubafrenzy holds for show_start / show_end markers (e.g.
  // "START OF SHOW: DJ Aubrey Hearst SIGNED ON at 7:43 PM (6/2/26)") rather than
  // reducing it to the bare DJ name. Marker text shape is owned by the writer (see
  // epic #1288); this module stays shape-agnostic about marker templates.
  describe('isMessageEntryType', () => {
    it.each(['breakpoint', 'talkset', 'message'])('returns true for %s', (entryType) => {
      expect(isMessageEntryType(entryType)).toBe(true);
    });

    it.each(['track', 'show_start', 'show_end', 'dj_join', 'dj_leave'])('returns false for %s', (entryType) => {
      expect(isMessageEntryType(entryType)).toBe(false);
    });
  });
  ...
  describe('resolveMessage', () => {
    it('returns null for a track entry (text belongs in artist_name)', () => {
      expect(resolveMessage('Stereolab', 'track')).toBeNull();
    });

    it('returns null for a null rawArtistName', () => {
      expect(resolveMessage(null, 'breakpoint')).toBeNull();
    });

    it('routes breakpoint text into message', () => {
      expect(resolveMessage('--- 7:00 PM BREAKPOINT ---', 'breakpoint')).toBe('--- 7:00 PM BREAKPOINT ---');
    });

    it('routes talkset text into message', () => {
      expect(resolveMessage('TALKSET', 'talkset')).toBe('TALKSET');
    });

    it('routes message text into message', () => {
      expect(resolveMessage('PSA: pledge drive starts Monday', 'message')).toBe('PSA: pledge drive starts Monday');
    });

    it('truncates message text to 250 chars', () => {
      const long = 'M'.repeat(300);
      const result = resolveMessage(long, 'talkset');
