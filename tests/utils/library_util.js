...
/**
 * Searches the library with anonymous auth (public endpoint).
 *
 * @param {object} searchParams - Search parameters
 * @param {string} [searchParams.query] - Combined search query
 * @param {string} [searchParams.artist] - Artist name
 * @param {string} [searchParams.title] - Album title
 * @param {number} [searchParams.limit] - Number of results
 * @param {string} anonymous_token - Anonymous auth token
 * @returns {Promise<object>} Search results
 */
exports.publicSearchLibrary = async (searchParams, anonymous_token) => {
  const params = new URLSearchParams();
  if (searchParams.query) params.append('query', searchParams.query);
  if (searchParams.artist) params.append('artist', searchParams.artist);
  if (searchParams.title) params.append('title', searchParams.title);
  if (searchParams.limit) params.append('limit', searchParams.limit.toString());

  const res = await fetch(`${url}/library/search?${params.toString()}`, {
    method: 'GET',
    headers: {
