 ...
 *   - 144 missing columns across 22 doubles (`missingColumns`)
 *
 * Worst offenders: `library_artist_view` 25, `digital_asset` 18, `library` 16,
 * `library_identity` 13, `artists` 9.
 *
 * Six doubles are still declared bare `{}` and appear here with their full
 * column list: `anonymous_devices`, `user_activity`, `specialty_shows`,
 * `schedule`, `artist_crossreference`, `cronjob_runs`.
 ...
 * qualified, which is why they are absent from this list. `genres` was filled
 * in upstream by BS#2410's #2445 and left the same way.)
 */
...
export const DB_MOCK_ALLOWLIST: DbMockAllowlist = {
  ...
  missingColumns: {
    ...
    genres: ['description', 'plays', 'add_date', 'last_modified'],
    library: [
      'genre_id',
      'format_id',
    ...
    rotation: ['discogs_release_id_source', 'lml_identity_id'],
