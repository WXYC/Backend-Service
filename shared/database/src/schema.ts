...
export const library = wxyc_schema.table(
  'library',
  {
    ...
    // Denormalized from artists.artist_name. Nullable until A.2 backfills
    // the column from the existing artist_id join, after which A.3 keeps it
    // current (live writes + cascade on artist rename). A.5 reads it.
    artist_name: varchar('artist_name', { length: 128 }),
    // STORED GENERATED tsvector covering the two searchable text columns
    // with weight bands (artist=A, album=B). Backed by a GIN index so the
    // service rewrite in A.5 can use `search_doc @@ websearch_to_tsquery`
    // for the dj-site default `Both` mode without joining artists.
