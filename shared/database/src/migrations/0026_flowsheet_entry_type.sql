...
SET entry_type = CASE
  WHEN message IS NULL OR message = '' THEN 'track'
  WHEN message LIKE 'Start of Show:%' THEN 'show_start'
  WHEN message LIKE 'End of Show:%' THEN 'show_end'
  WHEN message LIKE '% joined the set!' THEN 'dj_join'
  WHEN message LIKE '% left the set!' THEN 'dj_leave'
  ELSE 'message'  -- Legacy messages default to 'message' type
END;
