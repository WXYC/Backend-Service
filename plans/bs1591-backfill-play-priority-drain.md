...
## Post-review amendments (2026-07-16)

A 10-angle adversarially-verified code review of PR #1667 corrected three design errors in the plan above; the shipped code differs accordingly:

1. **The `stale_skipped` model above was inverted.** The CDC enrichment worker finalizes via `metadata_status` and never writes `metadata_attempt_at` (BS#891 design; the marker-stamping runtime fire-and-forget path has been dead code since Epic C C5 / #894), so worker-enriched rows can never drop out of a marker-only re-check — `stale_skipped` could only ever count hard deletes and out-of-band stamps, while the drain would silently re-enrich every worker-enriched row (duplicate LML fan-out, front-loaded to the same morning by the plays-DESC order). Shipped fix: `loadBatchByIds` also fetches `metadata_status` and the loop partitions on it — worker-terminal rows (`enriched_match` / `enriched_no_match` / `failed_no_retry`) get a marker-only reconcile stamp with zero LML calls (`worker_reconciled` bucket, the true overlap signal), in-flight `enriching` claims are left untouched (`worker_inflight_skipped`), and `stale_skipped` keeps only the vanished-id meaning.
2. **`BACKFILL_FLOOR_RECENCY_DAYS` default is 30, not 7.** A consumer-missed below-floor row sorts near the plays-DESC tail; a 7-day window could expire before any catch-up run reached it, permanently stranding it in the residual — the exact outcome decision 5 exists to prevent. The wide window is near-free post-amendment-1 (recent rows are almost all worker-enriched and reconcile without an LML call).
3. **Hardening:** loud `unwrapRows` on both work-list statements (a driver result-shape change must crash the run, not early-exit as a green zero-work cron — the fleet's `concerts-artist-resolver` convention); a positive-integer guard on the injectable `batchSize` seam (the work-list cursor, unlike the old `LIMIT` loop, would spin forever on 0); the eligibility EXISTS probes the join-bound `p.artist_norm` and runs after the free plays/recency arms.
