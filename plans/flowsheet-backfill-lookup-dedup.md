...
`enrich.ts` uses a conditional spread on the `'<field>' in artwork` witness for **all five track-aware URL columns** (`spotify_url`, `youtube_music_url`, `bandcamp_url`, `soundcloud_url`, `apple_music_url`). Present means LML decided, so we record it (with the `?? searchUrls.<field>` fallback for the four search URLs to handle LML-returned null, and bare `?? null` for `apple_music_url` per BS#1192). Absent means the cache stripped it, so we OMIT the column from both the `album_metadata` UPSERT (INSERT + `onConflictDoUpdate.set`) and the inline unlinked flowsheet UPDATE so the prior value is preserved.

The shape this guards is identical across all five columns. Naive `<field>: artwork.<field> ?? <fallback>` on a cache hit produces a write — `null` for `apple_music_url` (BS#1192: load-bearing destructive) or a per-row synthesized search URL for the four search URLs (BS#1338: verified-deep-link → per-row search-query degradation) — which the `album_metadata` UPSERT's `setWhere updated_at < NOW()` happily applies (the predicate always passes within a batch — R1's `updated_at` is microseconds in the past). The conditional spread makes the destructive overwrite impossible across the board for cache-hit rows: R1's verified URL survives instead of being clobbered by R2's synthesized or null value. R1 itself (cache miss) still records LML's decision on initial write.
...
- **Search URL cache-hit synthesis (BS#1338 follow-up).** Initially (ee0a609b), only `apple_music_url` used the conditional spread; the four search URLs (`spotify_url`, `youtube_music_url`, `bandcamp_url`, `soundcloud_url`) fell through `??` to per-row synthesized URLs on cache hits. This kept write-time parity with the runtime path (`metadata.service.ts#fetchMetadata`) and the worker (`apps/enrichment-worker/enrich.ts`) but degraded `album_metadata`'s shape: R2's per-row synthesized search URL would clobber R1's LML-verified deep-link via the same `setWhere updated_at < NOW()` mechanism that destroys nulled `apple_music_url`. BS#1338 extends the conditional-spread pattern to all five track-aware URL columns, so cache-hit rows preserve R1's verified URL on every column. The runtime and worker still synthesize at write time on first-write paths; the divergence is intentional and isolated to the backfill's cache-hit path. INSERT behavior (cache miss, including R1) is unchanged — the witness fires because LML's response carries the keys.
